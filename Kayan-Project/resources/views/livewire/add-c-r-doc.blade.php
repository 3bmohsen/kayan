...
<div class="d-flex flex-column flex-md-row " >
        @if ($doc->file_path != null)
          ...
          @endif
            ...
            <a href="" data-bs-toggle="modal" data-bs-target="#edit{{$doc->id}}" class="btn btn-primary m-3 mb-0">تعديل</a>
            <a href="" data-bs-toggle="modal" data-bs-target="#sure{{$doc->id}}" class="btn btn-danger m-3 mb-0">حذف</a>

                </div>
                <div class="d-flex flex-row">
                <p class="p-2">تاريخ العمل : {{ $doc->workDate }}</p>
                <p class="p-2">تاريخ الاستلام : {{ $doc->recDate }}</p>
                </div>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body text-center">
          هل انت متأكد من حذف {{$doc->file_name}}
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
          <a href="/del/CRDocs/{{$doc->id}}"><button type="button" class="btn btn-danger">حذف</button></a>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="edit{{$doc->id}}">تعديل</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
        <form class="row g-3 mt-5" action="/CensorshipDocs/edit/{{$doc->id}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
  @csrf
  <input required type="hidden" value="3" name="filetype">
  <div class="col-md-6">
    <label for="doc_file" class="form-label">ملف</label>
    <input type="file" class="form-control" id="doc_file" name="doc_file">
  </div>
  <div class="col-md-6">
    <label for="doc_name" class="form-label">اسم</label>
  ...
  <div class="col-md-6">
    ...
    <input required type="date" class="form-control" id="workDate" name="workDate" value="{{$doc->workDate}}">
  </div>
  <div class="col-md-6">
    <label for="recDate" class="form-label">تاريخ الاستلام</label>
    <input required type="date" class="form-control" id="recDate" name="recDate" value="{{$doc->recDate}}">
  </div>
  <div class="col-md-12">
