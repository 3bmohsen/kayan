        ...
        @if ($doc->file_path != null)
          ...
          @endif
            ...
            <div class="d-flex flex-row justify-content-lg-start justify-content-center p-3">
                    <a href="" data-bs-toggle="modal" data-bs-target="#edit{{$doc->id}}" class="btn btn-primary m-3 mb-0">تعديل النشاط</a>
                    <a href="" data-bs-toggle="modal" data-bs-target="#sure{{$doc->id}}" class="btn btn-danger m-3 mb-0">حذف النشاط</a>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body text-center">
          هل انت متأكد من حذف النشاط {{$doc->file_name}}
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
          <a href="/del/Activity/{{$doc->id}}"><button type="button" class="btn btn-danger">حذف النشاط</button></a>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="edit{{$doc->id}}">تعديل النشاط</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
        <form class="row g-3 mt-5" action="/Activity/edit/{{$doc->id}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
  @csrf
  ...
  <div class="col-md-6">
    ...
    <input required type="text" class="form-control" id="fileName" name="fileName" value="{{$doc->file_name}}">
  </div>
  <div class="col-md-12">
  ...
  <div class="col-12 text-center mb-5">
    <button type="submit" class="btn btn-primary">تعديل النشاط</button>
  </div>
</form>
        </div>
        <div class="modal-footer">
