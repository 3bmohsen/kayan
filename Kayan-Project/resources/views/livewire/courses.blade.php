...
<div class="d-flex flex-column flex-md-row " >
        @if ($doc->file_path != null)
          ...
          @endif
            ...
            <a href="" data-bs-toggle="modal" data-bs-target="#edit{{$doc->id}}" class="btn btn-primary m-3 mb-0">تعديل الدورة</a>
            <a href="" data-bs-toggle="modal" data-bs-target="#sure{{$doc->id}}" class="btn btn-danger m-3 mb-0">حذف الدورة</a>

                </div>
                <p class="p-2">التاريخ : {{$doc->expDate}}</p>
                <div class=" p-2">
                <p class="">الملاحظات: </p>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <div class="modal-body text-center">
          هل انت متأكد من حذف الدورة {{$doc->file_name}}
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
          <a href="/del/Course/{{$doc->id}}"><button type="button" class="btn btn-danger">حذف الدورة</button></a>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="edit{{$doc->id}}">تعديل الدورة</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
        <form class="row g-3 mt-5" action="/Course/edit/{{$doc->id}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
  @csrf
  <input required type="hidden" value="1" name="filetype">
  <div class="col-md-6">
    <label for="doc_file" class="form-label">ملف الدورة</label>
    <input type="file" class="form-control" id="doc_file" name="doc_file">
  </div>
  <div class="col-md-6">
    <label for="doc_name" class="form-label">اسم الملف</label>
    <input required type="text" class="form-control" id="doc_name" name="doc_name" value="{{$doc->file_name}}">
  </div>
  <div class="col-md-12">
    <label for="expDate" class="form-label">التاريخ</label>
