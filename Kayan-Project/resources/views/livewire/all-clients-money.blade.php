...
<div class="d-flex flex-column flex-lg-row align-items-center justify-content-center mb-5">
  ...
  <div class="container-fluid mt-2 ">
      <input required class="form-control me-2 text-center" wire:model="code" type="search" name="search" id="search" placeholder="البحث بإستخدام الكود" aria-label="Search">
  </div>
  <div class="container-fluid mt-2 ">
  ...
  <div class="container-fluid mt-2 ">
  <select wire:model="mission" name="search" id="search" class="form-control me-2 text-center" aria-label="Search">
    <option selected value="">إختر المأمورية</option>
...
{{$mon->links('vendor.pagination.bootstrap-5')}}

</div>
@foreach ($mon as $mo )
<div class="modal" id="sure{{$mo->id}}" tabindex="-1" aria-labelledby="sure{{$mo->id}}" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="sure{{$mo->id}}">تحذير</h5>
        ...
        <div class="modal-footer">
          ...
          <a href="/del/ClientMoney/{{$mo->id}}"><button type="button" class="btn btn-danger">حذف الحساب</button></a>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal" id="edit{{$mo->id}}" tabindex="-1" aria-labelledby="edit{{$mo->id}}" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="edit{{$mo->id}}">تعديل  الحساب</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
        <form class="row g-3 mt-5" action="/ClientMoney/edit/{{$mo->id}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
  @csrf
  <div class="col-md-6">
    <label for="rec_amount" class="form-label">المبلغ المستلم</label>
    <input required type="text" class="form-control" id="rec_amount" value="{{ $mo->rec_amount }}" name="rec_amount">
  </div>
  <div class="col-md-6">
    <label for="spent_amount" class="form-label">المبلغ المصروف</label>
    <input required type="text" class="form-control" id="spent_amount" value="{{ $mo->spent_amount }}" name="spent_amount">
  </div>
  <div class="mb-3">
  <label for="notes" class="form-label">ملاحظات</label>
  <textarea required  class="form-control" name="notes" id="notes" rows="3">{{ $mo->notes }}</textarea>
</div>
