...
<table class="table text-center table-hover" style="direction: rtl;">
  <thead>
    <tr>
      ...
      <th scope="col">ايميل المدير</th>
      <th scope="col">حذف المدير</th>
      <th scope="col">تعديل المدير</th>
...
<div class="modal" id="sure{{$admin->id}}" tabindex="-1" aria-labelledby="sure{{$admin->id}}" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="sure{{$admin->id}}">تحذير</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
          هل انت متأكد من حذف المدير {{$admin->name}}
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إغلاق</button>
          <a href="/del/admin/{{$admin->id}}"><button type="button" class="btn btn-danger">حذف المدير</button></a>
        </div>
      </div>
    </div>
  </div>
...
<div class="modal" id="edit{{$admin->id}}" tabindex="-1" aria-labelledby="edit{{$admin->id}}" aria-hidden="true">
<div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
          <h5 class="modal-title fs-5 text-center cairo-bold" id="edit{{$admin->id}}">تعديل المدير</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
        <div class="modal-body text-center">
        <form class="row g-3 mt-5" action="/admin/edit/{{$admin->id}}" method="POST">
        @method('PUT')
  @csrf
  <div class="col-md-6">
    <label for="inputEmail4" class="form-label">الاسم</label>
    <input required type="text" class="form-control" id="name" name="name" value="{{$admin->name}}">
  </div>
  <div class="col-md-6 justify-content-center">
    <label for="email" class="form-label">الايميل</label>
    <input required type="text" class="form-control" id="email" name="email" value="{{$admin->email}}">
  </div>
  ...
  <div class="col-12 text-center mb-5">
    <button type="submit" class="btn btn-primary">تعديل المدير</button>
  </div>
</form>
        </div>
        <div class="modal-footer">
